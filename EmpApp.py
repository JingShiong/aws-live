...
def Emp():
    ...
    try:
        cursor.execute(insert_sql, (emp_id, first_name,
                                    last_name, pri_skill, location, check_in))
        db_conn.commit()
...
def checkIn():
    ...
    try:
        cursor.execute(
            update_stmt, {'check_in': formatted_login, 'emp_id': int(emp_id)})
        db_conn.commit()
...
def checkOut():
    ...
    try:
        ...
        LogininDate = datetime.strptime(
            formatted_login[0], '%Y-%m-%d %H:%M:%S')
        ...
        try:
            cursor.execute(insert_statement, (emp_id,
                           formatted_login[0], formatted_checkout, Total_Working_Hours))
            db_conn.commit()
    ...
    return render_template("AttendanceOutput.html", date=datetime.now(), Checkout=formatted_checkout, LoginTime=formatted_login[0], TotalWorkingHours=Total_Working_Hours)
